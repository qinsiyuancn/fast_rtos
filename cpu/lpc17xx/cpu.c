...
static init_pll_con(const struct PLL * pll, unsigned long con)
{
    *pll->con = con;
    *pll->feed = 0xAA;
    *pll->feed = 0x55;
...
static do_init_pll(const struct PLL * pll)
{
    *pll->cfg = PLL0CFG_Val;
    init_pll_con(pll, 1);
    while(*pll->state & (1 << plock));
    init_pll_con(pll, 3);

}
static void init_pll()
{
    const struct PLL pll[] = {
	                         {&LPC_SC->PLL0CFG, &LPC_SC->PLL0CON, &LPC_SC->PLL0FEED, &LPC_SC->PLL0STAT, 26},
	                         {&LPC_SC->PLL1CFG, &LPC_SC->PLL1CON, &LPC_SC->PLL1FEED, &LPC_SC->PLL1STAT, 10},
                             };
    unsigned int i = 0;
    LPC_SC->CLKSRCSEL = CLKSRCSEL_Val;        /* Select Clock Source for PLL0       */
    while(i < (sizeof(pll)/sizeof(pll[0]))){
        do_init_pll(pll + i++);
    }
}
...
static unsigned int getFrequencyDefault()
{
    const unsigned int clock[] = clock_init();
    return clock[LPC_SC->CLKSRCSEL & 0x03] / ((LPC_SC->CCLKCFG & 0xFF)+ 1);
}

static unsigned int getFrequencyWhenPLL()
{
    if (((LPC_SC->PLL0STAT >> 24) & 3) == 3) {
        const unsigned int clock[] = clock_init();
        return (clock[LPC_SC->CLKSRCSEL & 0x03] * 
...
static unsigned int getFrequencyFromeRegister()
{
    ...
    unsigned int i = 0;
    for(i = 0; i < (sizeof(init_fun)/sizeof(init_fun[0]); i++)) {
        ret = init_fun[i]();
	if(ret) return ret;
    }
    return IRC_OSC;
