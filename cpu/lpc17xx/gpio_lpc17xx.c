...
static void set_gpio_on(unsigned char port, unsigned int mask)
{
...
static void set_gpio_off(unsigned char port, unsigned int mask)
{
   	if(port < sizeof(gpio_s)/sizeof(gpio_s[0])){
		if(gpio_s[port]){
			gpio_s[port]->FIODIR |= mask;
...
static unsigned int get_gpio(unsigned char port, unsigned int mask)
{
	if(port < sizeof(gpio_s)/sizeof(gpio_s[0])){
		if(gpio_s[port]){
			gpio_s[port]->FIODIR &= ~mask;
